    ...
    IAccessRestriction
{
    ...
    bytes32 public constant KAIZEN_CONTRACT_ROLE =
        keccak256("KAIZEN_CONTRACT_ROLE");
    ...
    /// @inheritdoc IAccessRestriction
    function ifKAIZENContract(address _address) external view override {
        require(isKAIZENContract(_address), "Caller not KAIZEN contract");
    }
    ...
    /// @inheritdoc IAccessRestriction
    function ifDataManagerOrKAIZENContract(address _address)
        external
        view
        override
    {
        require(
            isDataManager(_address) || isKAIZENContract(_address),
            "Caller not dm or tc"
        );
    }
    ...
    /// @inheritdoc IAccessRestriction
    function isKAIZENContract(address _address)
        public
        view
        override
        returns (bool)
    {
        return hasRole(KAIZEN_CONTRACT_ROLE, _address);
    }
