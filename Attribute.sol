...
contract Attribute is Initializable, IAttribute {
    ...
    /** NOTE modifier for check msg.sender has KaizenContract role */
    modifier onlyKaizenContract() {
        accessRestriction.ifKaizenContract(msg.sender);
        _;
    }
    ...
    /** NOTE modifier to check msg.sender has data manager or Kaizen contract role */
    modifier onlyDataManagerOrKaizenContract() {
        accessRestriction.ifDataManagerOrKaizenContract(msg.sender);
        _;
    }
    ...
    function reserveSymbol(uint64 _uniquenessFactor)
        ...
        override
        ifNotPaused
        onlyDataManagerOrKaizenContract
    {
    ...
    function setAttribute(
        ...
        uint64 _coefficient
    ) external override ifNotPaused onlyDataManagerOrKaizenContract {
    ...
    function createSymbol(
        ...
        uint8 _funderRank,
        uint8 _generationType
    ) external override onlyKaizenContract returns (bool) {
     ...
     * @param _funderRank rank of funder based on trees owned in Kaizen
